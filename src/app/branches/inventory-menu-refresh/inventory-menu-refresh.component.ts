...
@Component({
  selector: 'app-inventory-menu-refresh',
  templateUrl: './inventory-menu-refresh.component.html',
  styleUrls: ['./inventory-menu-refresh.component.scss']
})
export class InventoryMenuRefreshComponent {
  ...
  submit() {
    if (this.Form.valid) {
      ...
      this.dialogService.confirmed().subscribe(confirmed => {
        if (confirmed) {
          this.httpService.get('admin/inventory-menu-refresh/' + this.Form.value['from_branch_id'] + '/' + this.Form.value['to_branch_id'])
            .subscribe(result => {
              if (result.status == 200) {
